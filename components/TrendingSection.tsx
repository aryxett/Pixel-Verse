    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${source === "rawg" ? "bg-emerald-400" : "bg-slate-500"}`} />
          {source === "rawg" ? "Live from RAWG.io" : "Local data"}
        </div>
        {source === "rawg" && (
          <span className="text-xs text-slate-600">with Metacritic scores</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {source === "rawg"
          ...
          : games.map((game, i) => {
              // Use local GameCard for local data
